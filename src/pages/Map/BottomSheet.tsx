...
function BottomSheet({
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    if (timeDelta > 0) {
      ...
      const velocity = xDelta > Math.abs(yDelta)
        ? 0
        : (yDelta / timeDelta) * 1000; // px/s
  ...
  const handleTouchEnd = useCallback((_e: React.TouchEvent) => {
    ...
    if (lastVelocity > FLING_VELOCITY_THRESHOLD) {
      ...
      bottomSheetRef.current.style.setProperty('transition-duration', '0.15s');
      setTop(closedTop);
      onClose();
      return;
    }
  ...
  const handleHandleTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    // 핸들바 영역이 아니면 무시
    if (!handleBarRef.current || !handleBarRef.current.contains(e.target as HTMLElement)) {
      return;
    }
      ...
      >
        <SheetContainer 
          ...
          onTouchStart={(e) => {
            ...
            if (!handleBarRef.current || !handleBarRef.current.contains(target)) {
              e.stopPropagation();
              e.nativeEvent.stopImmediatePropagation();
              e.preventDefault();
            }
