...
function MeetingList ({ meetings }: MeetingListProps) {
        ...
        return (
            ...
            <ListHeader>
                <OptionInfo>
                    ...
                    <Arrow style={{ cursor: "pointer" }} onClick={() => setDropdownOpen(!dropdownOpen)} />
                    {dropdownOpen && (
                        <Dropdown>
                        <DropdownItem onClick={() => { setRecruitState('모집 중'); setDropdownOpen(false); }}>
                            모집 중
                        </DropdownItem>
                        <DropdownItem onClick={() => { setRecruitState('모집 완료'); setDropdownOpen(false); }}>
                            모집 완료
                        </DropdownItem>
                        </Dropdown>
                    )}
    ...
    ? meetings
        .filter(m => (recruitState === '모집 중' ? !m.complete : !!m.complete))
        .filter(m => (selectedType === '전체' ? true : m.type === selectedType))
        .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
    : [];  

    const previewMeetings = showAll ? filteredMeetings : filteredMeetings.slice(0, 2);
    ...
    return(
        <Layout>
            ...
            {filteredMeetings.length === 0 ? (
                ...
                <ListUp>
                {previewMeetings.map((meeting, idx) => (
